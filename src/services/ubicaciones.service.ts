...
function parseCodigoPostal(
  ...
  if (!Number.isInteger(n) || n < 0) {
    return { error: "codigo_postal must be a valid integer" };
  }
  return { value: n };
}
...
function parsePorDefecto(
  ...
  if (value === "false" || value === 0 || value === "0")
    return { value: false };
  return { error: "por_defecto must be a boolean" };
}

function validateCreateBody(
  body: Record<string, unknown>,
): ServiceResult<ValidatedCreateDto> {
  const direccion = parseOptionalString(
    body.direccion,
    "direccion",
    MAX_DIRECCION,
    true,
...
function validateUpdateBody(
  body: Record<string, unknown>,
): ServiceResult<ValidatedUpdateDto> {
...
function parseLocationId(raw: string): ServiceResult<number> {
  ...
  if (isNaN(id) || id < 1) {
    return { success: false, status: 400, error: "Invalid location ID" };
  }
...
export async function deleteUbicacion(
  ...
  if ((count ?? 0) > 0) {
    return {
      success: false,
      status: 409,
      error: "Cannot delete location because it has an active order",
    };
  }
  ...
  return {
    success: true,
    status: 200,
    data: { message: "Location deleted", id: idResult.data },
  };
}
